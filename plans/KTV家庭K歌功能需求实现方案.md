...
- 已补齐播放结束/退出总结面板、UltraStar / MIDI / KAR 导入、URL 导入、在线谱源搜索、自定义 GitHub 谱源、USDB / UltraStar-ES 可选 Provider。
- 已实现 App 内轻量补位：无人工谱但有同步歌词时，可从歌词时间轴生成并缓存 UltraStar RAP 节奏评分谱；它只判断节奏/参与度，不判断真实音高。
- 已实现实验性本地音高谱生成：用户触发或自动补位时，使用 Android 解码 + YIN + CQT/统计后处理思路，从原曲音频和歌词时间窗估算粗略目标音高，并缓存为 UltraStar 音符轨。它用于“没有人工谱时尽量可玩”，不承诺专业准确。
- 已修复音高谱显示抖动：音高线坐标固定在整首歌的 pitch scale，不再随当前窗口动态重映射；同一条音高线不应在播放接近时上下跳。
- 已补齐麦克风链路基础优化：`AudioRecord` 输入增加 200-3500 Hz 级联 IIR 滤波、动态底噪门限、关闭系统 AGC/回声消除/降噪，并在起播/seek 后短暂热身，减少手机麦克风录到扬声器声音后误评分。
- 已优化音高线反馈：金色音符表示 UltraStar 加分音符；普通音符在游标接触后立即出现高亮和已播放段，命中为绿色，唱偏或未稳定识别为金色，不再等到音符中段才变化。
- 已加入轻量娱乐互动特效：命中时显示脉冲反馈，Perfect / 颤音时显示金色或青色粒子，增强 K歌反馈但不遮挡主歌词。
- 已针对自动生成音高谱过碎问题补强：原曲分析加入轻量人声频段滤波；每个歌词时间窗保留多个音高候选，再用 Viterbi 式连续路径选择减少八度跳变和局部抖动；对过密逐字歌词自动合并成更稳定的音符段。
- 已继续参考 UltrastarCreatorTool 的后处理思路：短于约 220ms 的生成片段会吸收到邻近稳定音符，连续无音高/RAP 片段会合并成较长块，避免界面出现一串几乎连续的小短横线。
- 已修复评分谱生成/导入/清除后当前 K歌状态不刷新的问题：手机端和电视端都会强制重新加载当前绑定谱，不再需要手动关闭再开启 K歌模式。
- 当前不做重型“播放时自动扒专业音符轨”。这类功能即使用传统信号处理也容易慢、不准、误导评分；当前只保留轻量实验能力，准确评分仍优先使用人工 UltraStar / MIDI / KAR 评分谱。
...
- 2026-07-01 追加调研结论：Frank Karaoke、AllKaraoke、USDX、Vocaluxe、Performous 的共同高价值点集中在“轻量麦克风前处理、播放状态同步、逐行/逐音符反馈、远程手机麦克风”，而不是端侧重型自动制谱。因此当前优先优化麦克风抗串音、seek 后冻结评分、谱面质量反馈和远程麦克风可扩展性。
...
5. 已实现“轻量节奏评分谱”：从当前歌词时间轴生成 UltraStar RAP 类型评分窗口，用于无人工谱时的节奏/参与度评分，不做目标音高判断；播放中检测到可用同步歌词时会自动生成并缓存。
...
- 已完成：结果弹窗改为可视化面板，分数、等级、演唱覆盖、有效时长、连击和逐句统计更适合手机/TV 快速阅读；无评分谱时明确提示这是自由唱娱乐估算。
...
已实现一个不依赖重模型、不解码原曲 PCM 的 App 内补位方案：当当前播放项没有人工 UltraStar / MIDI 评分谱、但已经有带时间轴的歌词时，播放中会自动生成并缓存轻量节奏评分谱；用户也可在“评分谱”菜单手动生成。
...
- 明确边界：这是歌词时间轴生成的节奏谱，不是从原曲音频提取出的目标音高轨；结果面板会显示“节奏评分”，不显示“音准命中率”。
...
### 第七阶段补充：App 内轻量音高评分谱生成（已实现基础版）

已实现一个端侧实验方案：在当前歌曲已经有时间轴歌词、但没有人工 UltraStar / MIDI 评分谱时，用户可以手动生成带音高的 UltraStar 评分谱。实现仍坚持手机/电视可承受的轻量路线，不引入 Demucs、Whisper、CREPE、PyTorch 等重模型。

- 输入：当前播放音频 + 当前歌词时间轴。
- 解码：使用 Android `MediaExtractor` / `MediaCodec` 本地解码 PCM。
...
- 这是“轻量娱乐级扒谱”，不是专业主旋律提取。原曲里伴奏、人声混响、和声、强节奏乐器都会影响 YIN 结果。
- 不做人声分离，因此复杂歌曲可能仍会局部偏平、跳音或失败。
- 适合给无谱歌曲提供可玩的草稿；真实 UltraStar / MIDI / 人工谱仍然优先级最高。
...
6. 默认不自动触发原曲音频扒谱；允许用户手动生成轻量音高评分谱，并允许同步歌词自动生成轻量节奏评分谱作为补位。
7. TV 端优先引导手机作为麦克风。
...
7. 已完成：播放结束评分面板高对比视觉重做，手机/电视共用结果卡，区分音准评分、节奏评分和自由唱。
8. 已完成基础版：App 内本地轻量音高评分谱生成，带生成进度、缓存、短音符合并和时间线平滑。
9. 后续决策：TV 专用 K歌布局、手机当 TV 麦克风、多人评分。
