...
- 已实现 App 内轻量补位：无人工谱但有同步歌词时，自动从歌词时间轴生成并缓存 UltraStar RAP 节奏评分谱；它只判断节奏/参与度，不判断真实音高。
- 当前仍不做手机/电视端默认“原曲音频自动扒目标音高”。这类功能即使用传统信号处理也容易慢、不准、误导评分，后续只适合作为外部制谱工具或用户主动实验能力。
...
6. 默认不启用原曲音频自动扒谱；允许同步歌词自动生成轻量节奏评分谱作为补位。
7. TV 端优先引导手机作为麦克风。
...
8. 后续决策：TV 专用 K歌布局、手机当 TV 麦克风、多人评分。
