...
### 第二阶段：无音符轨娱乐跟唱评分（下一步优先）

目标：没有 UltraStar / MIDI 时也有完整 K歌体验，且不误导用户这是音准评分。

任务：

- 新增 `FreeSingScorer` 或扩展 `KaraokeScorer` 的无谱分支。
- 用歌词时间窗统计演唱覆盖率、节奏参与度、音量稳定度、音高稳定度、歌句完成度。
- 有逐字歌词时使用 word window；只有行级歌词时使用 line window；无同步歌词时不给高置信娱乐分。
- 播放开始、恢复播放、seek 后增加 2-5 秒 warmup，只校准环境底噪，不累计评分。
- 暂停、拖动、切歌、退出时冻结或重置当前分数，避免噪声污染。
- 播放结束或退出 K歌时显示结果弹窗：娱乐跟唱分、演唱覆盖率、节奏、稳定度。
- UI 文案明确显示“自由唱 / 娱乐跟唱分”，不显示“音准命中率”。
...
- 待完善：UI 显示目标音符条、用户音高线、命中率、连击、结束结果。
- 待完善：普通音符、金色音符、自由音符、Rap 音符的权重和展示。
- 待完善：文件选择器 / 分享打开 / URL 导入 UltraStar `.txt` 并缓存到歌曲 key。
- 待评估：MIDI / KAR 主旋律轨导入。
...
1. 无谱娱乐跟唱分。
2. 播放结束/退出 K歌结果弹窗。
3. 评分状态的 pause / seek / 后台重置和 warmup。
4. UltraStar 导入、绑定和缓存。
5. 有谱评分 UI：目标音符条、用户音高线、命中率。
6. TV K歌布局和手机当麦克风。
